...
通过发射激光测距生成三维点云的传感器。不受光照影响，能实测精确距离，但没有颜色/纹理信息，暴雨大雾天气下激光会被散射导致精度下降。原始数据格式是"点云"（见下），不是规则网格，因此不能直接用图像处理架构（CNN/ViT）处理，需先体素化（Voxel）或用专门点云网络（如 PointNet、SST）转换。
→ 首次问到：*EMMA 局限性讨论（跨论文）*（2026-09-11），详见 [sensor-fusion-qa.md](sensor-fusion-qa.md)

**点云 / Point Cloud**
LiDAR 的原始数据格式：一组离散的三维点，每个点通常包含 (x, y, z) 坐标 + intensity（反射强度），有时还有 ring（哪一线激光束）、timestamp。和图像的"稠密规则网格"相反，点云是"稀疏不规则散点"——3D 空间大部分位置没有点，且近处密、远处稀（激光束放射状发出）。
→ 首次问到：*EMMA 局限性讨论（跨论文）*（2026-09-11），详见 [sensor-fusion-qa.md](sensor-fusion-qa.md)

**毫米波雷达 / Radar**
通过发射毫米波并利用多普勒效应直接测量物体速度的传感器。穿透雾霾/沙尘能力强、探测距离远，但空间分辨率很低，难以分辨物体形状/类别。常和摄像头、LiDAR 三者互补融合。
→ 首次问到：*EMMA 局限性讨论（跨论文）*（2026-09-11），详见 [sensor-fusion-qa.md](sensor-fusion-qa.md)

**传感器融合 / Sensor Fusion**
把摄像头、LiDAR、雷达等不同传感器的数据结合处理，目的不是让数据格式变得统一（那只是技术手段），而是利用不同传感器失效场景互相错开这一点，让感知结果由多个独立证据源共同支撑。融合方式本身很关键：DriveMLM 让 LiDAR 表征"模仿"摄像头 CLIP 特征、消融显示几乎无提升；ARTEMIS 直接把两种模态投影进同一 BEV 空间共同处理，是更常见有效的做法。
→ 首次问到：*DriveMLM*（消融实验），概念展开：*EMMA 局限性讨论（跨论文）*（2026-09-11），详见 [sensor-fusion-qa.md](sensor-fusion-qa.md)
