...
**常识泛化能力 / Common-sense Generalization(接入大模型 vs 传统模型的真正区别)**
容易被误解为"传统模型是人提前写死的预设结果,大模型才是真正在推理"——这不准确。传统深度学习模型(UniAD、TransFuser、ARTEMIS 感知模块等)也是从数据里学出来的,不是硬编码规则(只有 DriveMLM 配的传统模块化系统里的规划器部分才真含规则)。真正的区别在**学习知识的来源和范围**:传统模型只在驾驶专用数据范围内学习,遇到训练数据没覆盖的组合(长尾场景)不是"给错预设答案",而是从没学过任何能泛化到这种情况的概念;接入大模型(VLM/LLM)的方案,知识来自海量互联网图文预训练,驾驶数据只是微调,所以能靠"迁移"通用常识去推理没见过的场景。呼应 DriveVLM 原文:"传统系统擅长'看见物体、预测轨迹',不擅长'理解语义、做常识推理'。"
**代价/反面**:灵活推理的思维链(CoT)有"不忠实解释"(unfaithful explanation)风险——模型说出来的推理文字不一定真的是它内部决策依据,可能是事后编的合理化文本,安全关键场景里是隐患。规则/传统系统虽不灵活,但行为完全可预测、可审计、可验证,这是量产系统安全认证还离不开传统路线的重要原因,不只是"技术不够先进"。DriveVLM-Dual 的"慢系统(VLM推理)+ 快系统(传统规划器兜底执行)"设计,某种程度上就是想同时保留这两个优点。
→ 首次问到:*EMMA 局限性讨论 / 传统vs大模型推理方式讨论(跨论文)*(2026-09-11)
