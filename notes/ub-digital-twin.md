...
### 3.5 LiDAR 点云在这篇论文里的具体三种用法

除了 Algorithm 1 的时序修复,论文里 LiDAR 点云还承担了另外两个同样具体的角色,值得单独拆开讲——这三种用法合在一起,正好是"LiDAR 除了装在车上实时开车用,还能怎么用"这个问题的一份完整案例(详细的跨论文对比见 [sensor-fusion-qa.md](../sensor-fusion-qa.md)):

**① 当测绘工具,重建物理测试场的三维几何(Map Reconstruction)。** OSM 路网/卫星图精度不够,论文用自己的自动驾驶车平台(2017 Lincoln MKZ)搭载的 LiDAR + 相机在测试场(Proving Ground)实地采集数据,具体流程:
- 用 **DLIO**(Direct LiDAR-Inertial Odometry,直接激光雷达-惯性里程计)对连续帧做实时拼接定位,合成覆盖约 1 平方公里的高密度点云(PCD 文件);
- 用 CloudCompare 去噪 + 下采样;
- 用 **Poisson Surface Reconstruction**(泊松表面重建)把离散点云转成三维网格(mesh),导出 FBX 格式——这一步正是"点云稀疏不规则、不能直接用,需要先转换成规则表示"这个问题的一个具体解法(和体素化 voxelization 是同类问题的不同解法,一个是给神经网络用、一个是给 3D 渲染引擎用);
- 把网格导入 MathWorks RoadRunner,当成"绝对几何参照物"——依据点云给出的道路边缘、地形起伏、建筑轮廓去对齐 DEM 高程数据,并精准摆放植被/路标/建筑(摄影测量法重建)等场景资产。

**② 当真值(Ground Truth),量化验证数字孪生的几何精度。** 用真实物理点云做基准,对两种模型分别做 ICP(Iterative Closest Point,迭代最近点)比对:

| 数据模型对象 | ICP RMSE | 平均点云间距(Mean Cloud-to-Cloud Distance) | 结论 |
|---|---|---|---|
| 网格化点云模型(Meshed Point Cloud) | 0.635 m | 0.183 m | 与真实物理环境几何一致性很高 |
| CARLA 渲染地图 | 1.831 m | 1.077 m | 中等程度偏差,主因是物体摆放/表面细节/植被表示的微小误差 |

这组数字是"闭环仿真到底需要多真实的传感器仿真"(EMMA 附录 A.5 提到但完全没展开的那个问题,见 [emma.md](emma.md))的一份具体量化答案——不是空口说"我们的仿真很真实",是拿真实点云当真值实测出来的。

**③ 当实时闭环仿真里的合成传感器数据流。** 数字孪生建好之后,CARLA 根据重建的 3D 地形反过来**模拟真实 LiDAR 的激光发射与回波**,生成合成点云流(10–15 Hz),通过 ROS2 桥接层(`autoware_carla_interface`)实时发布给 Autoware,供其内部的建图、**NDT 定位**(Normal Distributions Transform,一个纯几何配准算法,不涉及语义推理,和 Autoware 论文里描述的是同一套定位算法)与障碍物检测使用——这就是图解读里 Figure 3 那条"CARLA(Sync)渲染传感器数据→Autoware 处理"的具体内容。
