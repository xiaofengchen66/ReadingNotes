...
6. **Table 4:CoT 各组件的消融实验(§3.3)。** 单独加场景描述(Scene description)提升 0%;单独加关键物体(Critical object)提升 1.5%;单独加元决策(Meta decision)提升 3.0%(单项贡献最大);关键物体+元决策一起提升 5.7%;四项全加提升 6.7%(最优)。说明这四段 CoT 不是互相替代关系,是**逐步叠加、各自独立贡献**的——尤其是"元决策"这种抽象层级的推理,单独看比"关键物体"这种具体物体定位贡献还大,值得注意。

7. **Figure 4:数据规模缩放曲线(§3.3)。** 用内部海量数据集的 3%/12%/30%/100% 分别训练,横轴训练算力(FLOPs,对数刻度),纵轴 eval perplexity(越低越好)。四条曲线一开始重合下降,但数据越少的曲线越早"掉头"往上(过拟合信号)——3% 数据最早掉头,12%、30% 依次更晚,而 100% 数据在整个测试算力范围内(到 1.5×10²¹ FLOPs)都没有掉头,最终困惑度做到全图最低的 44.69。论文原句:"driving quality has not saturated when using the full large-scale dataset."——意思是目前限制 EMMA 表现的不是算力或模型容量,而是数据量,如果有更多数据,理论上还能继续变好。这张图是"自监督标签设计"(不需要人工标注,行车记录自带未来轨迹标签)在实际 scale up 时的实证回报:正因为标注免费,才能喂进去这么大的数据集去做这个缩放实验。

## English at-a-glance summary

### Figure 2 · EMMA Generalist(论文原图)

![EMMA Figure 2: Generalist illustration](img/emma_figure2.png)

左侧任务 prompt(未来动作/3D检测/道路图/是否封路等)→中间环视摄像头视频输入→EMMA(基于 Gemini 微调)→右侧统一用纯文本生成答案(轨迹坐标/检测框/车道点/是否封路的文字回答)→最右侧把文本答案解码叠加回原图做可视化。这张图最直观地体现了"驾驶即视觉问答"这个核心设计:不管任务是规划、检测还是问答,输出格式都是同一种纯文本,靠 prompt 区分任务类型。

### Table 4 + Figure 4 · 关键实验证据(重绘)

![EMMA summary poster: CoT ablation and data scaling](img/emma_summary.svg)

### 图解读 Diagram walkthrough

左侧对应论文 **Table 4**(CoT 消融):展示四段推理(场景描述/关键物体/元决策/行为描述)分别和组合加入时对规划质量的提升幅度——元决策单独贡献最大(+3.0%),四项全加达到 +6.7%。

右侧对应论文 **Figure 4**(数据规模缩放曲线):四条不同数据量(3%/12%/30%/100%)的训练曲线,数据越少越早过拟合(曲线掉头向上),100% 数据在测试算力范围内没有饱和迹象,最终困惑度做到全图最低的 44.69。
